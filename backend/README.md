...
4. If needed, edit the root-owned file with `sudoedit /etc/aerosspace-contact.env`. Fill in the existing SMTP host, port, security (`ssl` for port 465 or `starttls` for port 587), login, password, sender address, and notification address. Keep file mode `0600`, then run the installer again. Do not paste the password into chat, GitHub, or the website files.
